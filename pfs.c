...
void* search_dir(void* args){
	...
	//Check if the next thing is actually a directory...
	struct stat next_stat;
	while (next_dir != NULL){
		printf("Checking : %s\n", next_dir->file_name);
		if(stat(next_dir->file_name, &next_stat) != 0){
	...
	if (next_dir != NULL){
		...
		printf("next_dir is NULL\n");
	}
	if (queue->front !=NULL){
		printf("On deck in queue: %s\n", queue->front->file_name);
	}
	
	//Unlock the queue
	pthread_mutex_unlock(&q_lock);

	//if next_dir is not null
	if(next_dir != NULL){
